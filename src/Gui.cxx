...
void Gui::edit_selection( const int & selection_idx ){
    ...
    Eigen::Matrix4f delta;
    delta.setIdentity();
    //m_view->m_camera->m_model_matrix.data()
    Eigen::Affine3f selection_pose = Eigen::Affine3f::Identity();
    selection_pose.translation() << m_selection_points[selection_idx].x(), m_selection_points[selection_idx].y(), 1;

    ImGuizmo::Manipulate(view.data(), proj.data(), m_selection_guizmo_operation, m_selection_guizmo_mode, selection_pose.data(), delta.data() );

    //update the model matrix with the delta and updates the model matrix of all the children
    //cam->transform_model_matrix(Eigen::Affine3f(delta));

    const Eigen::Affine3f new_proj = Eigen::Affine3f ( delta ) * selection_pose;
    //Eigen::Vector3f Camera::project( pt, view, new_proj.matrix(), m_view->m_viewport_size);

    m_selection_points[selection_idx] = new_proj.translation().head<2>().cast<double>();
}
