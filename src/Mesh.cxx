...
void Mesh::transform_vertices_cpu(const Eigen::Affine3d& trans, const bool transform_points_at_zero){
    if ( trans.isApprox(Eigen::Affine3d::Identity()) ) // dont do useless work.
    {
        m_is_dirty = true;
        m_is_shadowmap_dirty = true;
        return;
...
void Mesh::read_ply(const std::string file_path){
    ...
    std::shared_ptr<tinyply::PlyData> vertices, normals, texcoords, color, intensity,  faces;
    ...
    try { intensity = file.request_properties_from_element("vertex", { "intensity" }, 3); }
    catch (const std::exception & e) { has_intensity=false; }
    ...
    typedef Eigen::Matrix<uint16_t,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> RowMatrixXu;
    ...
    //intensity
    if (has_intensity){
        if (intensity->t == tinyply::Type::UINT8) {
            Eigen::Map<RowMatrixXuc> mf( (unsigned char*)intensity->buffer.get(), intensity->count, 1);
            I=mf.cast<double>();
            I=I.array()/255.0;
        }if (intensity->t == tinyply::Type::UINT16) {
            Eigen::Map<RowMatrixXu> mf( (uint16_t*)intensity->buffer.get(), intensity->count, 1);
            I=mf.cast<double>();
            I=I.array()/255.0;
        }else if (intensity->t == tinyply::Type::FLOAT32) {
            Eigen::Map<RowMatrixXf> mf( (float*)intensity->buffer.get(), intensity->count, 1);
            I=mf.cast<double>();
        }else{ LOG(FATAL) <<"intensity parsing other than unsigned char and float not implemented yet"; }
    }
    //faces
    if (has_faces){
...
void Mesh::write_ply(const std::string file_path){
    ...
    //intensity
    if(I.size()){
        ply_file.add_properties_to_element("vertex", { "intensity" }, tinyply::Type::FLOAT32, I_f.rows(), reinterpret_cast<uint8_t*>(I_f.data()), tinyply::Type::INVALID, 0);
    }
