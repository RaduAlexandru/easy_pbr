...
Eigen::Matrix4f Camera::proj_matrix(const Eigen::Vector2f viewport_size){
    ...
    if (m_use_fixed_proj_matrix){
        ...
        {
            return compute_ortho_projection_matrix(-viewport_size.x()/2, viewport_size.x()/2,-viewport_size.y()/2.,viewport_size.y()/2, m_near, m_far);
        }
        else
...
Eigen::Matrix4f Camera::compute_ortho_projection_matrix(const float left, const float right, const float bottom, const float top, const float znear, const float zfar){
    //https://registry.khronos.org/OpenGL-Refpages/gl2.1/xhtml/glOrtho.xml

    Eigen::Matrix4f P;
    P.setConstant(0.0);

    const double drl = m_ortho_scale*(right-left);
    const double prl = m_ortho_scale*(right+left);
    const double dtb = m_ortho_scale*(top-bottom);
    const double ptb = m_ortho_scale*(top+bottom);
...
void Camera::mouse_scroll(const float x, const float y){
    ...
    if ( m_use_ortho_projection )
        m_ortho_scale = std::min(1.0,std::max(m_ortho_scale*val,1e-10));
    else
        push_away(val);
}
